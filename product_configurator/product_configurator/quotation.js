frappe.ui.form.on("Quotation Item", {
	...
	"clear_configuration": function(frm, doctype, docname) {
		product_configurator.clear_configuration(frm, doctype, docname);
	},
	
	// before removing the item row, clear the associated configuration
	"before_items_remove": function(frm, doctype, docname) {
		product_configurator.clear_configuration(frm, doctype, docname);
	}
});

frappe.provide("product_configurator");
$.extend(product_configurator, {
	get_product_configuration_for_item: function(frm, item) {
	...
	get_configuration_template: function(frm, item) {
	...
	open_configurator_dialog: function(frm, item, r) {
	...
	process_product_configuration: function(frm, item, dialog, product_configuration_specs) {
	...
	clear_configuration: function(frm, doctype, docname) {
		...
		// get the configurations for the above item
		var configs = product_configurator.get_product_configuration_for_item(frm, item);
		
		// remove the configuration from the client side datastore i.e. locals
		$.each(configs, function(i, config) {
			frappe.model.clear_doc(config.doctype, config.name);
		});
		
		// refresh field causes re-rendering of these fields
		frm.refresh_field("product_configuration");
		frm.refresh_field("product_configuration_section");
	}
});